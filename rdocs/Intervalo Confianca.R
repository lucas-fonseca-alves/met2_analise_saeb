...
#transformando varia´vel ano

saeb <- saeb %>%
  mutate(ANO_NASC_CARACT = case_when(
    ANO_NASC <= 2001 ~ "2001 ou antes",
    ANO_NASC == "1998 ou antes" ~ "2001 ou antes",
    ANO_NASC > 2001 ~ "2002 ou depois"
  ))
...
#------------------------------------------------------------------------------#
#                   Associação entre Variávies                                 #
#------------------------------------------------------------------------------#-----


# 1. A partir de sua amostra dos resultados do SAEB 9o. ano, considere amostras de tamanho 15 e 200. 
# Para cada amostra, explore a associação entre as seguintes variáveis:
#   
#   - NOTA_MT e LOCALIZAÇÃO (Urbana e Rural)
#   - NOTA_LP e Ano de nascimento (2001 ou antes ; 2002 ou depois)
# Para avaliar essas relações construa os gráficos adequados e medidas de posição e 
# variabilidade segundo categorias das variáveis qualitativas. Você diria que a proficiência 
# em matemática é maior em escolas urbanas? Existe diferença entre as proficiências em língua portuguesa 
# segundo grupo de idade do estudante? Considere os testes: t de Student, Wilcoxon-Mann-Whitney e 
# Kolmogorov-Smirnov. (Não se esqueça de testar normalidade e homocedasticidade - fazer referência, se 
# vc já analisou esses aspectos anteriormente)



#NOTA_MT e LOCALIZAÇÃO (Urbana e Rural) -> boxplot


# Subamotra 15

subAmostra_n200 <- na.omit(subAmostra_n200)
  

#Medidas de posição

rural_200 <- subAmostra_n200 %>% 
  filter(LOCALIZACAO == "Rural") 

describe(rural_200$NOTA_MT)

urbana_200 <- subAmostra_n200 %>% 
  filter(LOCALIZACAO == "Urbana")

describe(urbana_200$NOTA_MT)

ggplot(subAmostra_n200) +
  aes(x = LOCALIZACAO , y = NOTA_MT) +
  geom_boxplot (fill = "#88B4E7", width = 0.5) +
  stat_summary (
    fun = "mean", geom = "point", shape = 23, size = 3, fill = "white"
  ) +
  labs(x = "Localização", y = "Notas em Matemática")


ggsave("box_subamostra200_notaMT-LOCALIZACAO_analise_4.2.pdf", width = 158, height = 93, units = "mm")



#Hipotese: Normalidade alfa = 5%

#H0: Notas em Matemática seguem distribuição normal
#H1: Notas em Matemática não seguem distribuição normal

    shapiro.test(subAmostra_n200$NOTA_MT)
    # Shapiro-Wilk normality test
    # 
    # data:  subAmostra_n200$NOTA_MT
    # W = 0.99215, p-value = 0.3581
    # 


#Não rejeita H0 -> Logo segue uma distribuição normal


#hipotese:  homocedasticidade

#H0: As variâncias são iguais
#H1: As variâncias são diferentes

var.test(rural_200$NOTA_MT, urbana_200$NOTA_MT)
    # F test to compare two variances
    # 
    # data:  rural_200$NOTA_MT and urbana_200$NOTA_MT
    # F = 0.88462, num df = 28, denom df = 170, p-value = 0.7277
...
#Não rejeita H0 -> Aas variancias são iguais


#Hipótese:

#H0: Notas em Matemática em escolas urbanas são menores ou iguais que as notas em matematica em escolas rurais
#uu <= ur

#H1: Notas em Matemática em escolas urbanas são maiores que as notas em matema´tica em escolas rurais
#uu > ur

t.test(rural_200$NOTA_MT, urbana_200$NOTA_MT, alternative = "greater")
t.test(NOTA_MT ~ LOCALIZACAO, data = subAmostra_n200, alternative = "less")
        # Welch Two Sample t-test
        # 
        # data:  NOTA_MT by LOCALIZACAO
        # t = -0.3026, df = 40.121, p-value = 0.3819
        # alternative hypothesis: true difference in means between group Rural and group Urbana is less than 0
        # 95 percent confidence interval:
        #   -Inf 11.8481
        # sample estimates:
...
#Resultado: não rejeita H0, logo as Notas em Matemática em escolas urbanas são menores ou iguais que as notas em matematica em escolas rurais


#Wilcoxon-Mann-Whitney 

wilcox.test (NOTA_MT ~ LOCALIZACAO, data = subAmostra_n200, alternative = "less")

    # Wilcoxon rank sum test with continuity correction
    # 
    # data:  NOTA_MT by LOCALIZACAO
    # W = 2117, p-value = 0.6041
...
ks.test(NOTA_MT ~ LOCALIZACAO, data = subAmostra_n200, alternative = "greater")
  
        # Exact two-sample Kolmogorov-Smirnov test
        # 
        # data:  NOTA_MT by LOCALIZACAO
        # D^+ = 0.1708, p-value = 0.208
        # alternative hypothesis: the CDF of Rural lies above that of Urbana
...
antes_2001_200 <- subAmostra_n200 %>% 
  filter(ANO_NASC_CARACT == "2001 ou antes") 

describe(antes_2001_200$NOTA_MT, na.rm = T)

depois_2002_200 <- subAmostra_n200 %>% 
  filter(ANO_NASC_CARACT == "2002 ou depois")

describe(urbana_200$NOTA_MT)

ggplot(subAmostra_n200) +
  aes(x = ANO_NASC_CARACT , y = NOTA_LP) +
  geom_boxplot (fill = "#88B4E7", width = 0.5) +
  stat_summary (
    fun = "mean", geom = "point", shape = 23, size = 3, fill = "white"
  ) +
  labs(x = "Nascimento", y = "Notas em Português")


ggsave("box_subamostra200_notaLP-AnoNasc_analise_4.2.pdf", width = 158, height = 93, units = "mm")
...
shapiro.test(subAmostra_n200$NOTA_LP)

# Shapiro-Wilk normality test
# 
# data:  subAmostra_n200$NOTA_LP
# W = 0.94982, p-value = 0.5578
...
var.test(antes_2001_200$NOTA_LP, depois_2002_200$NOTA_LP)

        # F test to compare two variances
        # 
        # data:  antes_2001_200$NOTA_LP and depois_2002_200$NOTA_LP
        # F = 0.90185, num df = 44, denom df = 133, p-value = 0.7086
        # alternative hypothesis: true ratio of variances is not equal to 1
        # 95 percent confidence interval:
        #   0.5710568 1.5124841
        # sample estimates:
...
#H0: Notas em Português em nascidos 2001 ou antes são iguais que as que nasceram em 2002 ou depois
#uu = ur

#H1: Notas em Português em nascidos 2001 ou antes são diferentes que as que nasceram em 2002 ou depois
#uu != ur


 t.test(NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n200)

 # Welch Two Sample t-test
 # 
 # data:  NOTA_LP by ANO_NASC_CARACT
 # t = -3.4562, df = 79.235, p-value = 0.0008835
 # alternative hypothesis: true difference in means between group 2001 ou antes and group 2002 ou depois is not equal to 0
 # 95 percent confidence interval:
 #   -36.89195  -9.92870
 # sample estimates:
 #   mean in group 2001 ou antes mean in group 2002 ou depois 
 # 236.6633                     260.0736 
 # 
...
wilcox.test (NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n200)

    # Wilcoxon rank sum test with continuity correction
    # 
    # data:  NOTA_LP by ANO_NASC_CARACT
    # W = 2075, p-value = 0.001785
...
ks.test(NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n200)

      # Exact two-sample Kolmogorov-Smirnov test
      # 
      # data:  NOTA_LP by ANO_NASC_CARACT
      # D = 0.2864, p-value = 0.005918
...
subAmostra_n15 <- na.omit(subAmostra_n15)


#Medidas de posição

rural_15 <- subAmostra_n15 %>% 
  filter(LOCALIZACAO == "Rural") 

describe(rural_15$NOTA_MT)

urbana_15 <- subAmostra_n15 %>% 
  filter(LOCALIZACAO == "Urbana")

describe(urbana_15$NOTA_MT)

ggplot(subAmostra_n15) +
  aes(x = LOCALIZACAO , y = NOTA_MT) +
  geom_boxplot (fill = "#88B4E7", width = 0.5) +
  stat_summary (
    fun = "mean", geom = "point", shape = 23, size = 3, fill = "white"
...
ggsave("box_subamostra15_notaMT-LOCALIZACAO_analise_4.2.pdf", width = 158, height = 93, units = "mm")
...
shapiro.test(subAmostra_n15$NOTA_MT)
# Shapiro-Wilk normality test
# 
# data:  subAmostra_n15$NOTA_MT
# W = 0.93939, p-value = 0.4105
...
var.test(rural_15$NOTA_MT, urbana_15$NOTA_MT)
# F test to compare two variances
# 
# data:  rural_15$NOTA_MT and urbana_15$NOTA_MT
# F = 0.9693, num df = 3, denom df = 9, p-value = 0.8974
# alternative hypothesis: true ratio of variances is not equal to 1
# 95 percent confidence interval:
#   0.1908773 14.0287253
# sample estimates:
#   ratio of variances 
# 0.9692978 
...
t.test(rural_15$NOTA_MT, urbana_15$NOTA_MT, alternative = "greater")
    # t.test(NOTA_MT ~ LOCALIZACAO, data = subAmostra_n15, alternative = "less")
    # Welch Two Sample t-test
    # 
    # data:  NOTA_MT by LOCALIZACAO
    # t = 0.66998, df = 5.6653, p-value = 0.7354
    # alternative hypothesis: true difference in means between group Rural and group Urbana is less than 0
    # 95 percent confidence interval:
    #   -Inf 73.67582
    # sample estimates:
...
wilcox.test (NOTA_MT ~ LOCALIZACAO, data = subAmostra_n15, alternative = "less")
...
ks.test(NOTA_MT ~ LOCALIZACAO, data = subAmostra_n15, alternative = "less")

# Exact two-sample Kolmogorov-Smirnov test
# 
# data:  NOTA_MT by LOCALIZACAO
# D^+ = 0.1, p-value = 0.8941
# alternative hypothesis: the CDF of Rural lies above that of Urbana
...
antes_2001_15 <- subAmostra_n15 %>% 
  filter(ANO_NASC_CARACT == "2001 ou antes") 

describe(antes_2001_15$NOTA_MT, na.rm = T)

depois_2002_15 <- subAmostra_n15 %>% 
  filter(ANO_NASC_CARACT == "2002 ou depois")
...
ggsave("box_subamostra15_notaLP-AnoNasc_analise_4.2.pdf", width = 158, height = 93, units = "mm")
...
shapiro.test(subAmostra_n15$NOTA_LP)
...
var.test(antes_2001_15$NOTA_LP, depois_2002_15$NOTA_LP)

# F test to compare two variances
# 
# data:  antes_2001_15$NOTA_LP and depois_2002_15$NOTA_LP
# F = 0.95612, num df = 4, denom df = 8, p-value = 0.9612
# alternative hypothesis: true ratio of variances is not equal to 1
# 95 percent confidence interval:
#   0.1892314 8.5855276
# sample estimates:
...
t.test(NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n15)

# Welch Two Sample t-test
# 
# data:  NOTA_LP by ANO_NASC_CARACT
# t = -1.4661, df = 8.5548, p-value = 0.1784
# alternative hypothesis: true difference in means between group 2001 ou antes and group 2002 ou depois is not equal to 0
# 95 percent confidence interval:
#   -80.66168  17.52973
# sample estimates:
...
wilcox.test (NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n15)
...
ks.test(NOTA_LP ~ ANO_NASC_CARACT, data = subAmostra_n15)

# Exact two-sample Kolmogorov-Smirnov test
# 
# data:  NOTA_LP by ANO_NASC_CARACT
# D = 0.4, p-value = 0.5415
# alternative hypothesis: two-sided




# 4.2 - 2) Para a amostra de tamanho 15, verifique se há diferença entre as notas de língua portuguesa e 
# matemática. Considere os testes: t de Student, Wilcoxon e Sinais. Comente os resultados.
...
var.test(subAmostra_n15$NOTA_LP, subAmostra_n15$NOTA_MT)

# F test to compare two variances
# 
# data:  subAmostra_n15$NOTA_LP and subAmostra_n15$NOTA_MT
# F = 0.75597, num df = 13, denom df = 13, p-value = 0.6213
# alternative hypothesis: true ratio of variances is not equal to 1
# 95 percent confidence interval:
#   0.2426827 2.3548583
# sample estimates:
...
t.test(subAmostra_n15$NOTA_LP, subAmostra_n15$NOTA_MT)
    # Welch Two Sample t-test
    # 
    # data:  subAmostra_n15$NOTA_LP and subAmostra_n15$NOTA_MT
    # t = -0.031487, df = 25.507, p-value = 0.9751
...
wilcox.test (subAmostra_n15$NOTA_LP, subAmostra_n15$NOTA_MT)
...
ks.test(subAmostra_n15$NOTA_LP, subAmostra_n15$NOTA_MT)

# Exact two-sample Kolmogorov-Smirnov test
# 
# data:  subAmostra_n15$NOTA_LP and subAmostra_n15$NOTA_MT
# D = 0.21429, p-value = 0.9205
# alternative hypothesis: two-sided
